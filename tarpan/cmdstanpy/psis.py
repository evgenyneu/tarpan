...
def psis_pareto_k_plot(fit, name, values=None,
                       lpd_column_name=LPD_COLUMN_NAME_DEFAULT,
    ...
    values : list of float
        Values to be plotted on the x-axis that correspond to K values
        (plotted on the y-axis). Usually, these are
        observed vales. If not given, points are labeled 1, 2, 3 ...
        on the x-axis.
    ...
    return psis_pareto_k_plot_from_psis_data(
            psis_data=psis_data, name=name, values=values,
            pareto_k_plot_params=pareto_k_plot_params)


def psis_pareto_k_plot_from_psis_data(
        psis_data: PsisData, name, values=None,
        pareto_k_plot_params: ParetoKPlotParams = ParetoKPlotParams()):
    """
    Make a plot that shows values of Pareto K index generated by PSIS
    method. This is used to see if there are values of K higher than 0.7,
...
def save_psis_pareto_k_plot(
    ...
    fig, ax = psis_pareto_k_plot(
        fit=fit, name=name, values=values,
        lpd_column_name=lpd_column_name,
        pareto_k_plot_params=pareto_k_plot_params)

    model_name_sanitised = name.replace(" ", "_").lower()
